...
- vite 是一个面向未来的前端构建工具，开发阶段基于 esmodule 和 esbuild 构建，生产阶段通过 rollup 进行打包
- `开发阶段`
  - 冷启动快，充分利用浏览器原生基于 esmodule 的特点，不需要打包编译，并且利用上浏览器缓存
  - 通过 esbuild 对依赖包做预构建，减少 http 请求，对非 esm 模块转 esm 模块
    - esbuild 基于 go 语言开发，利用多线程
      - 优点
        - 加速依赖预构建(缓存判断、扫描、打包写入磁盘)
        - 合并模块，减少 http 请求
        - 对非 esm 模块转 esm 模块
      - 缺点
        - 对 css 分割不友好
  - 真正的`按需加载`，基于 esm 的 HMR 在模块发生变化时，只需要重新加载变动的模块就可以
- `生产阶段`
  - 通过 rollup 打包构建，esbuild 协助
- 弊端
