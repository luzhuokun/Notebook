...
- webRTC 工作流程
  - 本地`采集`
    - 添加媒体`轨道`（音频、视频）到对等对象上，获取摄像头/麦克风权限
    - 通过 rtcConnection.addEventListener('track',()=>{})捕获远程轨道
  - 创建`RTCPeerConnection`对象
  - 通过`信令服务器`交换 SDP（会话描述信息）、ICE（候选信息），交换双方信息并建立连接
    ...
    - `ICE`（Interactive Connectivity Establishment，即交互式连接建立）
      ...
      - STUN 服务收集 NAT 外网地址
      - TURN 收集中继地址
  - 最后通过建立数据通道（data channel）传递媒体流（MediaStream）
