...
* loader.js代码的正则的写法不易理解与维护
* loader.js代码中新增自定义规则困难
...
1. eslint的插件就是要以npm包的形式进行引入
2. 因为不涉及公司隐私问题，以公共包的方式更方便

!> 这里就不细讲eslint-plugin-gdds插件内的代码了，如果要新增规则的话，可以先看看这个人的[eslint-plugin 入门](https://www.jianshu.com/p/f3fddccb059a)，然后再看eslint-plugin-gdds里面的代码就知道怎么做了。更多细节可查看[eslint 文档](https://eslint.bootcss.com/docs/developer-guide/working-with-plugins)
          ...
          rules: [
              {
                  ...
                  loader: "eslint-loader" /* 之前这个地方也进行loader.js的规则，现在去掉了 */
              }, {
                  test: /\.vue$/,
                  exclude: /node_modules/,
                  /* 这里要先执行eslint-loader，因为里面包含了vue的一些语法检查 */
                  use: [{ loader: "./loader.js?dir=" + __dirname }, { loader: 'eslint-loader' }]
              }
          ]
      }
    }
...
?> 优化代码递交至 [重庆电信4.0](http://121.199.61.174:18080/svn/kids/branches/chongqin/iptv4.0) svn地址，更多细节可下载代码慢慢研究。
...
3. 编辑器安装eslint插件，在settings.json中加入以下配置
```js
...
- 不敢对代码进行UglifyJsPlugin优化，因为有些机顶盒子太烂，压缩后的语法不支持
