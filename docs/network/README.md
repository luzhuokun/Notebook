...
- 心跳是让了双方都知道对方还活着，在没有传输数据的时候 tcp 是不知道双方是否已经断开的，如果有一方断了，另一方就一直等造成资源浪费
- 通过 ping/pong 机制实现，客户端定时发送 ping 数据包，服务器响应 pong 包
...
- HTTPS 协议分为三个阶段：TCP 三次握手、TLS 四次握手、加密通信
