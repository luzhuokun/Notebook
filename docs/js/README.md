...
- 在代码执行前，js 引擎把作用域中 `var 变量声明移动到作用域顶部`创建，这个过程就是变量提升
- 在一个作用域中，在 var 声明和赋值之前访问这个变量，会返回 undefined
...
- 把构造函数设计成立即执行，可以尽快地确定 promise 的状态，更快地开始异步任务的操作，不需要额外的事件循环开销，如果构造函数是异步的，会拖延后续代码的执行
- 如果 then 设计成异步执行，`确保代码执行顺序的一致性`
  - 如果设计成同步，then 下面的代码有可能先执行，也有可能后执行，因为 then 的执行是根据 promise 的状态来决定的，promise 的状态确定是未知的，那么如果状态确定时不把后续代码放微任务执行，而是直接放调用栈中执行，那么保证不了 then 中代码先执行还是 then 外面代码先执行，就会有很多不确定因素，增加代码调试的难度
  - 如果设计成异步的话，这个执行顺序是一致的，先执行 then 下面的代码，再执行 then 的回调
