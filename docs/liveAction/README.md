...
- 最后下麦断流，离开频道
- 在斗鱼 sdk 没有提供声纹处理的情况下，自己手动在本地流中提取不同用户的声纹进行处理，解决不能监听音量变化的问题
...
- 全流程把控数据看板项目的落地，对自己的一次挑战，就基于 mildom 项目组的运营人员，希望可以有一个可视化数据平台来直观地看运营数据，在这样的背景下，我担任起数据看板前端项目的主要搭建工作
- 首先从`项目立项`开始，跟各部门负责人开会，确定项目目标、排期和人员分配
- 然后进行`技术调研和技术选型`，选择合适的技术栈进行开发
  - 当时选型 react+echarts+redux 状态库来进行系统搭建，通过 monorepo 来同时管理 web 端和 h5 端项目
- `撰写设计文档`
  - 列出功能清单、实现方案、架构设计
    - 写流程图、时序图等等
- 根据需求文档`拆分功能模块`
  - 分别拆分出基础模块、公共模块、业务模块（各种柱状图、饼状图、折线图、热力图等图表展示、排行榜榜单数据）
- 进入开发流程后，`建立 git 仓库`，根据 gitflow 流程协同开发，`制定编码规范`，通过 git 钩子在代码提交时进行检测，保证代码质量
- 解决开发中的难点痛点，比如在开发的过程中
  - `引入 web-vitals` 和尝试使用 sentry 收集页面性能和异常告警
  - `引入 rtk-query` 集中管理接口请求和状态，解决接口状态难管理的痛点
  - 还通过 `react18 的并发特性`优化用户频繁输入、视图切换场景
- 后面还配置 gitlab-ci 和 Jenkins 完成部署工作
- 最后经过一个多月的努力，最终完成项目上线
- 改进点
  - 针对数据格式转换适配，做单元测试（使用 jest，写测试用例）
    - 接口返回的数据需要做一定的转换适配，跟 echarts 要的数据格式不适配（饼状图、柱状图、折线图）
      - 接口一般是返回一个数组对象的结构，需要根据不同的图表转换成特定的数据结构作为入参
  - 时间格式转换
  - 持久化 localstorage 的封装
  - 实时数据更新

### 需求复盘会

- `针对`需求分析、设计、实现、测试以及上线后反馈等`各环节进行`系统性`回顾和总结的过程`，通过复盘持续地优化改进
- 在开发的过程中有没可以`优化开发效率`的地方
  - 构建工具优化
    - 优化 webpack 编译（关闭 ts 检查）
    - 考虑使用 vite 替换 webpack
- `减少 bug 出现`
  - 开会时会一起过一遍 jira 平台上面的 bug，问一下 bug 产生的原因，以及如何改进（需求理解不到位导致功能上出现问题）
  - 制定单元测试和错误代码检查减少 bug 出现的可能
- `减少沟通成本`
  - 与产品
    - 需求不够清晰（一句话、跟之前一样）
      - 新增一个新类型的礼物（聊天气泡）
  ...
  - 与后端
    ...
    - 功能分工不明确
      - 部分功能前端自己算
      - cms 跳转详情页（让前端自己组装数据带过去）
      - 一起玩功能，在用户付款后，通过前端轮训的方式去获取支付完成信息（可能改成 websocket 更合理）
  - 与测试
- `收集组内其他成员跟其他部门协作问题，然后在跨部门沟通时讨论改进

- 跟产品沟通
  - 需求定义不清晰，设计稿（有些交互逻辑不清晰）
  - 中途改需求
    - 如果改动不大，不影响正常的上线流程可以接受
    - 如果改动很大，就不接受，让他放到下一个版本里做，或者延期（底线）
- 跟后端沟通
  - 接口定义不够清晰
- 跟测试沟通
