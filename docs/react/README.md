...
- 解决函数组件没有状态和生命周期的问题
- 解决`状态复用`问题，class component 复用状态，需要通过高阶组件（HOC）来进行嵌套获取状态，在跨组件的状态需要复用的场景下，会嵌套很多层形成嵌套地狱不好维护，如果通过 hooks，不需要层层传递状态，直接调用相关的 hooks 函数就可以
- 解决`副作用`问题，在 class component 的生命周期场景中，容易聚集着大量不相关的副作用逻辑，而且难以拆解，越聚越多难以维护。在 function component 中我们可以通过 useEffect 把副作用代码独立放在不同的 useEffect 中执行
...
- 调度阶段(Scheduling)
  ...
  - `调度开始`后，根据任务优先级和时间分片(5ms)循环调度任务，每次到达分片时间，检查优先级执行更高优先级任务
...
- 合成事件是 react 模拟浏览器 DOM 事件流自己实现的一套事件处理机制，统一解决不同浏览器之间的差异
- react 的这套事件处理机制是通过事件委托的方式，统一在 document 上处理各种事件，并不是像 vue 那样把事件绑定在对应的 dom 上处理，不仅解决一致性问题，还减少了内存占用
- 在 react17 以后合成事件会被放到创建时的 root 根节点上，这样可以兼容更多的场景（挂载到根节点是因为可以同时在项目运行多个 react 版本不会互相影响，方便微前端架构接入）
- 在合成事件中会触发批处理机制
...
- `第一步`是先从左到右`遍历`，找出可以`复用`的节点，并判断属性是否有变化而打上`更新标记`
...
- fiber 的出现主要是为了解决在调和阶段同步渲染，长时间占用主线程，造成页面卡顿无法交互的问题
- fiber 是一个`链表树`数据结构，每个节点上包含了三个指针，分别指向他的兄弟节点、子节点以及父节点，可以方便回溯整个链表树
- 每一个 fiber 节点也是一个`执行单元`，在 react 的调度中，在开启了并发模式下，每构建完一个 fiber 节点就去检查一下有没超过 5ms 的时间分片，让主线程有机会去执行优先级更高的任务，实现了可中断渲染的机制，提高用户体验
...
- react hooks：函数式语法，为了保持简洁和纯函数，`保证调用顺序`，通过`链表`实现，每次重渲染都会重新执行一遍
...
- react18
  ...
  - `自动批处理`覆盖所有场景
  - SSR 支持 Suspense 组件的使用（通过 Suspense 动态注入，不需要等到所有组件加载完再注入）
