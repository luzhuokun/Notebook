...
- 去除未被使用的 js 代码，减少包体积
- 原理包括
  - 根据 esm 模块的静态特性分析，分析代码的 ast 抽象语法树以及模块导入和导出之间的关系
  - 基于依赖图，标记未使用代码
  - 去除未使用代码
...
- 浏览器接收到通知后，根据上一次的 hash 值，先是请求`hot-update.json`，然后对文件中的 hash 值进行存储，用于下一次使用，根据 chunkId 和上次的 hash ,通过 jsonp 的方式请求相关的 `hot-update.js` 文件进行加载
...
- 在面对真实的业务场景，通过上面的分包策略还是不够的，会出现有些包被多次导入的情况，因此我们需要更细颗粒度的分包
- 通过 splitchunks 插件来做更细颗粒度地分包，在实际项目中，我会把代码包拆分成：框架包、公共代码包以及业务包，通过配置 cacheGroups 分组、chunk 块大小、引用次数等信息来进行分包
  - `分组`（cacheGroups）
